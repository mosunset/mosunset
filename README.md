...
- **ステージングされていない変更**（修正されたが、まだステージングされていないファイル）
- **追跡されていないファイル**（新しく追加されたがGitがまだ管理していないファイル）
...
### push 後に リセットした場合、それをリモートに反映させる

リモートリポジトリのブランチを強制的に上書きします。`--force` オプションを使ってリモートリポジトリを更新します。
...
> **注意:**
> `--force` オプションは強制的にリモートの内容を上書きするので、他のチームメンバーが同じリポジトリを使っている場合、慎重に行ってください。他の人がリモートのリポジトリを基に作業していると、その変更が失われる可能性があります。
...
> 詳しい使い方は[branch.md#状況-2-開発中に他の人がmainブランチを更新した場合](./branch.md#状況-2-開発中に他の人がmainブランチを更新した場合)を参照
